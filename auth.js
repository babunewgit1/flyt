// !==============================================
//       !Auth.js for blackjet
// !==============================================

//  Remove redirect function from session storage
window.addEventListener("load", function () {
  sessionStorage.removeItem("redirectAfterLogin");
  sessionStorage.removeItem("redirectTo");
});

// Function to clear all pending session flags
function clearAllPendingFlags() {
  sessionStorage.removeItem("redirectAfterLogin");
  sessionStorage.removeItem("redirectTo");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  crossBtn.addEventListener("click", function () {
    ...
    // Clear all pending flags when user closes popup without logging in
    clearAllPendingFlags();
  });

  // Handle backdrop click to close popup
  if (authPopUpWrapper) {
    // Monitor popup visibility changes using MutationObserver
    // This handles cases where popup is hidden via inspect element or other methods
    const observer = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        if (
          ...
          // If popup was active but now is not active (hidden by any method)
          if (
            mutation.oldValue &&
            mutation.oldValue.includes("active_popup") &&
            !target.classList.contains("active_popup")
          ) {
            // Clear all pending flags when popup is hidden
            clearAllPendingFlags();
          }
        }
        // Also monitor style changes (display: none via inspect element)
        if (
          mutation.type === "attributes" &&
          ...
          // Check if popup is hidden via inline style changes (DevTools edits)
          const isInlineHidden =
            inlineStyle.display === "none" ||
            inlineStyle.visibility === "hidden";

          // Check if popup is hidden via computed styles
          const isComputedHidden =
            computedStyle.display === "none" ||
            computedStyle.visibility === "hidden";

          // If popup is hidden by any method, clear flags
          if (isInlineHidden || isComputedHidden) {
            clearAllPendingFlags();
          }
        }
      });
    ...
    // Start observing
    observer.observe(authPopUpWrapper, {
      attributes: true,
...
function trackCurrentPage() {
  try {
    ...
    // Automatically extract page name from URL path
    let pageName = "";
    ...
    if (currentPath === "/" || currentPath === "/home") {
      ...
      if (pathSegments.length > 0 && pathSegments[0]) {
        ...
        // Fallback to page title if no path segments
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (signupForm) {
    signupForm.addEventListener("submit", async (e) => {
      ...
      // validate phone Number with the dom plugin
      if (window.iti.isValidNumber()) {
        var internationalNumber = window.iti.getNumber(
      ...
      try {
        ...
        // Phone validation logic with abstractapi
        if (!verifyData.response.phone_valid) {
          showToastMessage("Please enter a valid phone number.");
          return;
        }

        // Email validation logic with abstractapi
        if (
    ...
    logoutBtn.addEventListener("click", async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Dispatch event to trigger reload in checkout pages
...
// Export page tracking functions to global scope
